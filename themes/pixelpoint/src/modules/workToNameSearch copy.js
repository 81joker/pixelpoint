...
class Search {
  ...
  typingLogic() {
    ...
    if (searchValue) {
      ...
      this.typingTimer = setTimeout(() => this.getResults(searchValue), 2000);
    } else {
  ...
  getResults(searchQuery = "", pageSize = 8, pageNumber = 1) {
    ...
    $.getJSON(apiUrl, function(data) {
      const filteredData = searchQuery
        ? data["@graph"].filter(item => item.name && item.name.toLowerCase().includes(searchQuery))
        : data["@graph"];
